...
\usepackage{bibentry}
% END REMOVE bibentry
%En commençant par la tâche 6, nous nous sommes rendu compte que le preprocessing de textes, comme la suppression des titres ou la supressions des balises url ou de mentions n'apportaient pas ou même faisait baisser la performance. Nous avons donc continué sans preprocessing sur les deux tâches en nous focalisant sur l'utilisation d'un modèle préentraîné adapté aux différences tâches.
%Compte tenu des 
\begin{document}
...
For Task 1, we fine-tuned an XLM-RoBERTa-Large model on multilingual data with class weighting, achieving an overall F1-score of 0.633 on the test set.
For Task 6, we utilized a DeBERTa-V3-Base model trained using 5-fold cross-validation with weighted loss, achieving an F1-score of 0.951 on the test set.
\end{abstract}
...
We describe our system configurations, training strategies, and results, demonstrating competitive performance, particularly for Task 6. While our approach primarily leverages established transformer architectures without novel methodological modifications, it demonstrates effectiveness under limited computational resources.
...
The training data for this task included previously published datasets in French and German, as well as Russian user reviews from the RuDReC corpus \cite{tutubalina2020russian}, and tweets from the SMM4H 2021 shared task \cite{magge-etal-2021-overview}. The English data was reused from earlier editions of the SMM4H shared tasks. The full dataset and shared task overview are described in \cite{smm4h-heard-overview-2025}.

We first provide a brief analysis of the Task 1 training and development data (Tables~\ref{tab:dataset-overview}–\ref{tab:pos-length}).
...
\multicolumn{2}{c|}{\textbf{Dev}} \\ \cline{2-5}
 & Total & Pos (\%) & Total & Pos (\%) \\
\hline
...
\centering
\setlength{\tabcolsep}{6pt}
\caption{Median and IQR of positive texts (label 1).}
\label{tab:pos-length}
\begin{tabular}{|l|cc|cc|}
\hline
\multirow{2}{*}{\textbf{Lang}} & \multicolumn{2}{c|}{\textbf{Train}} & \multicolumn{2}{c|}{\textbf{Dev}} \\ \cline{2-5}
 & Med & IQR & Med & IQR \\
\hline
de & 204 & 69–272 & 220 & 72–292 \\
fr & 155 & 97–203 & 118 & 73–177 \\
ru & 34 & 13–37 & 20 & 12–37 \\
en & 19 & 14–24 & 19 & 12–23 \\
\hline
\end{tabular}
\setlength{\tabcolsep}{6pt}
\normalsize
...
The dataset consisted of Reddit comments and titles joined with comments, reflecting the informal, varied, and conversational nature of online discourse. This setting posed challenges for accurate classification due to the variety of expressions and noise present in Reddit posts, which we aimed to address in our modeling strategy described below.
...
Final predictions on the test set were generated by ensembling. We averaged the predicted probabilities for the positive class obtained from the best model of each of the 5 folds. A final decision threshold was then optimized on these averaged probabilities using the true labels from the holdout set to maximize the F1-score for the positive class.

\subsection{Implementation Details}

All models were trained using the PyTorch framework \cite{pytorch2019}, and all experiments were run on a single NVIDIA RTX 3070 GPU with 8GB of VRAM.
...
Performance on Russian posts was lower in comparison, as reflected in the F1-score of 0.538.
...
% --- Conclusion ---
\section{Post-hoc Error Analysis}
...
The effectiveness of the no-preprocessing approach indicates that the model's attention mechanisms might adeptly focus on relevant text spans despite surrounding noise. 
...
This shows that a well-tuned and relatively lightweight setup can serve as a strong baseline, especially in resource-constrained scenarios.
